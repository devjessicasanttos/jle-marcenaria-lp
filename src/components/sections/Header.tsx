  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 lg:px-20 flex justify-between items-center relative">
        ...
        {/* LOGO */}
        <a href="#inicio" className="flex items-center gap-3 z-[10000]">
        ...
        <nav className="hidden lg:flex items-center gap-10">
          {navLinks.map((link) => (
            <a
              key={link.label}
              href={`#${link.id}`}
              className="font-montserrat text-white/90 hover:text-[#C62828] text-[11px] uppercase tracking-widest font-semibold transition-all"
            >
              {link.label}
            </a>
          ))}
        ...
        {/* BOTÃO MOBILE (ÍCONE DE LISTA) - FORÇADO E VISÍVEL */}
        <button
          className="lg:hidden z-[10000] bg-[#C62828] p-3 rounded-full text-white shadow-2xl active:scale-95 transition-all"
      ...
      <div
        className={`fixed inset-0 bg-[#121212] flex flex-col items-center justify-center gap-8 transition-all duration-500 lg:hidden ${
          isOpen ? "translate-x-0 opacity-100 z-[9998]" : "translate-x-full opacity-0 pointer-events-none"
        }`}
      >
        {navLinks.map((link) => (
          <a
